...
def train(config):
    ...
    try:
        ...
        # Scale
        if config["scale"]:
            df.loc[:, df.columns != target] = sklearn.preprocessing.MinMaxScaler().fit_transform(df.loc[:, df.columns != target])
        ...
        if spliter.n_folds > 1:
            mlflow.log_metrics(
                untensor_dict(dicts_mean(fold_metrics)), step=trainer.global_step  # type: ignore
            )
        ...
        _logger.exception(e)
        mlflow.end_run(
            mlflow.entities.RunStatus.to_string(mlflow.entities.RunStatus.FAILED)
        )
        raise
